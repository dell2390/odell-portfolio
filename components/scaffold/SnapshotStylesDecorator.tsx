...
*/
const getGenerateClassName = ({ kind, story }: SnapshotStylesProviderProps): GenerateId => (
    rule,
    styleSheet
) => {
    const key = slugify(
        `${kind}-${story}-${
            (styleSheet && styleSheet.options.classNamePrefix) || "NO STYLESHEET PROVIDED"
        }-${rule.key}`
    )
...
const SnapshotStylesProvider: FC<SnapshotStylesProviderProps> = ({ children, ...rest }) => {
    return (
        <StylesProvider generateClassName={getGenerateClassName(rest)}>{children}</StylesProvider>
    )
}
